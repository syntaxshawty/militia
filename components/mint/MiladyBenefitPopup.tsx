...
export default function MiladyBenefitPopup({
  onMinted,
  minimized,
  onMinimize,
}: Props) {
  ...
  const [claimedOptimistic, setClaimedOptimistic] =
    useState(false);
  ...
  const {
    ...
    ? {
        ...
        miladyTier: DEV_MOCK_MILADY,
        refetchEligibility: async () => {},
      }
    : real;
  const { quotedPrice, refetchPrice } = useQuoteMintPrice(
    1,
    address
  );
  const {
  ...
  const isClaimed =
    !!miladyBenefitClaimed || claimedOptimistic;
  const isVisible =
    (isConnected || !!DEV_MOCK_MILADY) &&
    (hasActiveBenefit || isClaimed);

  if (
    !isVisible ||
    minimized ||
    (isClaimed && !isMinimizing)
  )
    return null;

  const benefitLabel =
    miladyTier === "free"
      ? "You have a FREE mint available!"
      : miladyTier === "halfOff"
