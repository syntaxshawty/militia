...
export default function MintCard({
  ...
  const {
    ...
    priceError,
    refetchPrice,
  } = useQuoteMintPrice(quantity, address);
  const {
    mint,
    reset,
    receipt,
    isPending,
    isConfirming: realConfirming,
    isConfirmed,
    error,
  } = useMint();
  const isConfirming =
    DEV_MOCK_CONFIRMING || realConfirming;

  const [mintRank, setMintRank] = useState<Rank | null>(
    null
  );
  const [benefitConsumed, setBenefitConsumed] =
    useState(false);
  ...
  const mintedIds = receipt
    ? parseMintedTokenIds(receipt.logs)
    : [];
  ...
  useEffect(() => {
    ...
    if (
      tokenURIs.length !== mintedIds.length ||
      tokenURIs.some((u) => !u)
    )
      return;
    if (fetchTokenRank) {
      Promise.all(
        tokenURIs.map((u) => fetchTokenRank(u!))
      ).then((ranks) => {
  ...
  return (
    <div className="font-mono text-green-500 text-sm space-y-4 w-full">
      ...
      <div className="space-y-1">
        ...
        <div className="flex justify-between items-center">
          <span className="flex items-center gap-2">
            PRICE
            {bulkDiscount && (
              <span className="text-xs px-1 border border-yellow-500 text-yellow-500">
                {bulkDiscount}
              </span>
            )}
          </span>
          <span className="flex items-center gap-2">
            ...
            {(pricingState === "normal" ||
              pricingState === "discounted" ||
              pricingState === "fallback") &&
              effectivePrice !== undefined &&
              formatMintPrice(effectivePrice)}
            {pricingState === "discounted" &&
              basePrice !== undefined && (
                <span className="text-xs text-green-800 line-through">
                  {formatMintPrice(basePrice)}
                </span>
              )}
            {pricingState === "fallback" && (
              <span
      ...
      <div className="text-xs space-y-1">
        {paused && (
          <p className="text-green-700">MINTING PAUSED</p>
        )}
        {pricingState === "free" && (
        ...
        <p
          className={`text-green-600 ${pricingState === "discounted" ? "visible" : "invisible"}`}
        >
          DISCOUNT APPLIED
        </p>
        {pricingState === "fallback" && (
          <p className="text-yellow-600">
            ESTIMATED PRICE — LIVE QUOTE UNAVAILABLE
          </p>
        )}
      </div>
      ...
      {isConnected && (
        <div className="flex items-center gap-4">
          <button
            ...
            disabled={
              quantity <= 1 || isPending || isConfirming
            }
      ...
      {isConnected ? (
        <div className="flex gap-2">
          ...
          >
            {getMintLabel()}
            {isConfirming && (
              <span className="neon-sweep" aria-hidden />
            )}
          </button>
          {isConfirmed && (
            <button
