...
export function getMockQuotePrice(quantity: number): { price: bigint | undefined; error: string | null } {
  switch (ACTIVE_MOCK_STATE) {
    case "free":
      return { price: BigInt(0), error: null };
    case "halfOff":
      return { price: (MOCK_BASE_PRICE * BigInt(quantity)) / BigInt(2), error: null };
    case "priceFailure":
      return { price: undefined, error: "Simulated contract revert: quoteMintPrice failed" };
    case "normal":
    case "claimed":
    default:
      return { price: MOCK_BASE_PRICE * BigInt(quantity), error: null };
  }
}
...
export function getMockEligibility(): {
  ...
  switch (ACTIVE_MOCK_STATE) {
    case "free":
      return { canMintWithMilady: true, miladyBenefitClaimed: false, hasActiveBenefit: true, miladyTier: "free" };
    case "halfOff":
      return { canMintWithMilady: true, miladyBenefitClaimed: false, hasActiveBenefit: true, miladyTier: "halfOff" };
    case "claimed":
      return { canMintWithMilady: true, miladyBenefitClaimed: true, hasActiveBenefit: false, miladyTier: null };
    case "normal":
    case "priceFailure":
    default:
      return { canMintWithMilady: false, miladyBenefitClaimed: false, hasActiveBenefit: false, miladyTier: null };
  }
}
