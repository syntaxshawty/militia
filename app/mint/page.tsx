...
export default function Home() {
  ...
  const [tokenMetadata, setTokenMetadata] = useState<Record<string, unknown>[]>([]);
  ...
  const mintedKey = mintedIds.map((id) => id.toString()).join(",");
  useEffect(() => {
    ...
    Promise.all(
      mintedIds.map((id) =>
        fetch(`${gateway}/ipfs/${env.metadataCid}/${id.toString()}.json`)
          .then((r) => r.json())
          .catch(() => null)
      )
    ).then((results) => {
                ...
                <div className="mt-3 border border-green-900 p-4">
                  {tokenMetadata.length > 0 ? (
                    <pre className="text-xs text-green-500 whitespace-pre-wrap break-words leading-relaxed">
                      {JSON.stringify(tokenMetadata[carouselIndex] ?? tokenMetadata[0], null, 2)}
                    </pre>
                  ) : (
